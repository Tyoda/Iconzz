...
public class Iconzz implements WurmServerMod, Initable, Versioned, PreInitable, ServerStartedListener {
    ...
    /**
     * The directory of the mod relative to the server root
     */
    public static final String modDirectory = Paths.get("mods", "Iconzz").toString();

    /**
     * The temp dir relative to the server root, where some files are stored before init
     */
    ...
    public void preInit() {
        try{
            if(!preInited){
                ...
                Path resourcesPath = Paths.get(modDirectory, "resources");
                if(tempDir.exists()){
                ...
                for(String sName : sheetNamesInOrder) {
                    Path original = Paths.get(resourcesPath.toString(), sName);
                    Path nw = Paths.get(tempDir.toString(), sName);
                    Files.copy(original, nw);
                }

                Files.copy(Paths.get(resourcesPath.toString(), "mappings.txt"),
                           Paths.get(tempDir.toString(), "mappings.txt"));

                // Load images
                int sheetNum = 0;
                for(String imageName : sheetNamesInOrder){
                    sheetImages[sheetNum++] = ImageIO.read(Paths.get(tempDir.toString(), imageName).toFile());
                }
    ...
    public void init() {
        ...
        // remove images and freeSpaces from memory, as they won't be used anymore
        sheetImages = null;
        freeSpaces.clear();
        freeSpaces = null;
    ...
    public short addIcon(String name, String path){
        try {
            ...
            final short newId = freeSpaces.removeFirst();
            final int sheetNum = (int) newId / 240;
            final int x_offset = ((int)newId%20) * 32;
            final int y_offset = (((int)newId%240)/20) * 32;

            BufferedImage sheetImage = sheetImages[sheetNum];
     ...
     * @return the ID belonging to the icon, or -1 if no such name is registered
     */
